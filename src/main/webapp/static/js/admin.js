...
function processResult(result){
    ...
    var html = "";
    html+="<tr>";
    html+="<td class='name-td'>"+"<a href='javascript:window.location.hash=\"#"+getFileLink()+"\";processHash()'>"+".."+"</a>"+"</td><td></td><td></td><td></td>";
    html+="</tr>";
    for(var i=0;i<result.length;i++){
        html+="<tr>";
        if(!result[i].file){
            // if is folder
            html+="<td class='name-td folder-td'>"+"<a href='javascript:window.location.hash=\"#"+getFileLink(result[i].name)+"\";processHash()'>"+result[i].name+"</a>"+"</td>";
        }else{
            // if is file
            html+="<td class='name-td file-td'>"+"<a href='javascript:showImage("+i+")'>"+result[i].name+"</a>"+"</td>";
        }
...
function deleteImage(){
    ...
    $.ajax({
        url : '/admin/delete',
        type : 'POST',
        data:{year:year,month:month,day:day,name:image},
...
function list(year, month, day, func){
    if(!year&&!month&&!day){
        ...
        $.get("/admin/list",{year:year},function(result){
            processResult(result);
            if(func)
                func();
        });
        ...
        $.get("/admin/list",{year:year,month:month},function(result){
            processResult(result);
            if(func)
                func();
        });
        ...
        $.get("/admin/list",{year:year,month:month,day:day},function(result){
            processResult(result);
            if(func)
                func();
        });
